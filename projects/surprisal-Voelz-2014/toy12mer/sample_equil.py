...
usage = """Usage: sample.py infile.mtx microstates.dat TotalNumSamples NumTrials outName

    Will write a series of files:
        tCounts.outName.TotalNumSamples.0.mtx,
        ....
        tCounts.outName.TotalNumSamples.(NumTrials-1).mtx
"""
...
peq = np.exp(2.72*np.array(mInfo.microNumContacts))
peq /= peq.sum()
print 'Equilibrium probabilities', peq
print 'Native population:', peq[11811]
...
for trial in range(NumTrials):
    ...
    NumSamplesPerState = np.random.multinomial(nsamples, peq, size=1)[0]
    print 'NumSamplesPerState', NumSamplesPerState
    ...
    print 'Sampled', nsamples, 'according to equilibrium distribution.  TotalCounts =', TotalCounts

    mmwrite('tCounts.equil.%s.%d.%d.mtx'%(outName,nsamples,trial), C)

...
def reweightCountMatrix(C, sequence, MJdict, ContactStateIndices, microContactStates, beta, Verbose=True):
    if Verbose: print 'reweighting the transition matrix to reflect the sequence...'
    newC = C.tolil()
    rows, cols = newC.nonzero()
    for k in range(len(rows)):
        ...
        if ContactStateIndices[i] != ContactStateIndices[j]:
            ...
            newC[i,j] = newC[i,j]*min(1., np.exp(beta*(u_i-u_j)))
    return newC
...
############################
# generate microstate trajectories at a set of temperatures

ntemps = 8
dlog = (np.log(1.0) - np.log(0.5))/float(ntemps)
betas = np.exp(np.arange(np.log(1.0), np.log(0.5)-dlog, -dlog))
print 'betas', betas

jumpsByTemp = []

# for each temperature,
# build a dictionary of neighbor jumps
for temp in range(ntemps):

    jumpFn = 'jumps%d.pickle'%temp
    if os.path.exists(jumpFn):
        print 'Reading jump probabilities from:', jumpFn, '...'
        fin = open(jumpFn,'r')
        jumps = pickle.load(fin)
        jumpsByTemp.append( jumps )
        fin.close()    

    else:   
        # Reweight the transition matrix according to our chosen sequenece
        ...
        initial_sequence = ['Lys', 'Lys','Lys','Lys','Lys','Lys']
        ...
        newC = reweightCountMatrix(T, sequence, MJdict, ContactStateIndices, microContactStates, betas[temp])
        newT = EstimateTransitionMatrix(newC) 

        rows, cols = newT.nonzero()
        jumps = {} # {microstate index: [ jump_indices, jump_probs ] }
        print 'Compiling jump list...'
        lasti =  0 
        for k in range(len(rows)):
            i, j = rows[k], cols[k]
            if lasti != i:
              if i%1000 == 0:
                print 'state', i, 'of', len(microstates)
            if not jumps.has_key(i):
                jumps[i] = [[],[]]
            jumps[i][0].append(j)
            jumps[i][1].append(float(newT[i,j]))
            lasti = i
    ...
    if (0):
      print 'Checking self-transitions...'
      for k in range(len(microstates)): 
        print 'state', k, 'self-transition?', bool(jumps[k][0].count(k))
        if not bool(jumps[k][0].count(k)):
            print '!!!!!!!!!!'

    if (0):
      print 'Checking to see if sum(probs(j|i)) = ..'
      for k in range(len(microstates)):
        print 'state', k, 'sum of transition probs', sum(jumps[k][1])

# Generate successive MC jump rounds to increasingly populate a count matrix  
CountsByTemp = []
NumStates = len(microstates)
for temp in range(ntemps):
    CountsByTemp.append( scipy.sparse.lil_matrix((int(NumStates),int(NumStates)),dtype='float32') )

nsteps = 10000 # steps per trial  
ntrials = 1000
nswap = 1 # number of trials to try REMC temperature swaps
i = [0 for temp in range(ntemps)]
print '#ncounts\ttemp\tstability\tslowest\tnext-slowest'
for trial in range(ntrials):

    for temp in range(ntemps):

        C = CountsByTemp[temp]
        jumps = jumpsByTemp[temp]

        # add new counts to the matrix
        step = 0
        while step < nsteps:
            j = jumps[i[temp]][0][draw_index(jumps[i[temp]][1])]
            # if step%10000 == 0:
            #     print 'step', step, i, '->', j, microstates[j]
            C[i[temp],j] += 1
            i[temp] = j
            step += 1
        ...
        if EnoughCounts: 
            ...
            print '%d\t%d\t%8.3f\t%16.8f\t%16.8f'%(trial*nsteps, temp, stability, result[0,1], result[1,1])
        else:
            print '%d\tnot enough counts yet...'%(trial*nsteps)
